...
//
common.rmApp = function (appsDir, app, callback, packagesDir) {
  
//  return rimraf(path.join(appsDir, app.user, app.name), callback);
// giff start
  packagesDir = packagesDir || haibu.config.get('directories:packages');
  
  return rimraf(path.join(appsDir, app.user, app.name), function(err) {
	if(err) { return callback(err); }
	
	var dirs = [];
	var patt = new RegExp("^" + [app.user, app.name].join('-') ) ;
		
	fs.readdirSync(packagesDir).forEach(function (dir) {
		if( patt.test(dir) ) { dirs.push(dir); }
	});
	
	async.forEach(dirs, function rmCache (dir, next) {
      rimraf(path.join(packagesDir, dir), next);
    }, callback);
  });
// giff end
};
